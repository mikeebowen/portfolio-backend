...
describe('postContentItemEndpoint', () => {
  ...
  it('should send the error message and status status when errors are included', (done) => {
    ...
    Promise.resolve(postContentItemEndpoint(req, res))
      .then(() => {
        expect(res.status.calledWith(444), 'did not set status when responseData.status was included')
          .to
          .equal(true);
        expect(res.json.calledWith(req.responseData), 'did not send data when responseData.data was included ')
          .to
          .equal(true);
        done();
      })
